...
import { MapPin, Building2, Eye, Send, Package } from 'lucide-react';
    ...
    useEffect(() => {
        const fetchVendors = async () => {
            try {
                ...
                //  FIXED: was checking res.data.status === 'success' (wrong)
    ...
    return (
        <div className="bg-gray-50 flex flex-col min-h-screen">
            ...
            <div className="relative w-full h-43 flex items-center justify-center bg-cover bg-center" style={{ backgroundImage: "url('https://utkarshujjain.com/assets/img/banner-10.jpg')" }}>
                ...
                <h1 className="relative z-10 text-3xl md:text-4xl font-bold text-white tracking-wider text-center px-4">{t('pages.industry_service.title')}</h1>
            </div>

            {/* Services List Section */}
            <div className="max-w-7xl mx-auto px-4 -mt-8 sm:-mt-10 md:-mt-12 relative z-20 pb-20 w-full">
                {loading ? (
                    <div className="bg-white border-b-4 border-[#233480] shadow-xl p-6 space-y-4">
                        {[1, 2, 3].map((i) => (
                            <div key={i} className="animate-pulse flex flex-col md:flex-row items-center gap-4 p-5 bg-white border border-gray-100 rounded-lg">
                    ...
                    <div className="bg-white border-b-4 border-[#233480] shadow-xl p-6 text-center py-10 text-gray-500">
                        No industry services found.
                    </div>
                ) : (
                                    ...
                                    <div key={product._id || `${vendor._id}-${idx}`} className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow overflow-hidden border border-gray-200">
                                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 p-3 sm:p-4 md:p-6">
                                            ...
                                            >
                                                ...
                                                <span className="hidden sm:inline">{t('pages.vendor_details.send_enquiry')}</span>
                                                <span className="sm:hidden">{t('pages.vendor_details.send_enquiry')}</span>
                                            </button>
                                        </div>
                                    </div>
                                ))
